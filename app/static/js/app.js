...
// Naplnenie kategórií do filtra
function populateCategoryFilters() {
    const mainCategoryFilter = document.getElementById('mainCategoryFilter');
    mainCategoryFilter.innerHTML = '<option value="">Všetky kategórie</option>';
    ...
    // Vyčisti podkategórie
    const subcategoryFilter = document.getElementById('subcategoryFilter');
    subcategoryFilter.innerHTML = '<option value="">Všetky podkategórie</option>';
    subcategoryFilter.disabled = true;
}

// Naplnenie kategórií do formulára
function populateEntryCategories() {
    const entryMainCategory = document.getElementById('entryMainCategory');
    entryMainCategory.innerHTML = '<option value="">Vyber hlavnú kategóriu</option>';
    ...
    entrySubcategory.innerHTML = '<option value="">Vyber podkategóriu</option>';
    entrySubcategory.disabled = true;
}

// Aktualizácia podkategórií vo filtre
async function updateSubcategoryFilter() {
    ...
    try {
        ...
        console.error('Chyba pri načítavaní podkategórií:', error);
        subcategoryFilter.disabled = true;
    }
}

// Aktualizácia podkategórií vo formulári
...
async function loadYears() {
    try {
        ...
        if (data.years) {
            ...
            yearFilter.innerHTML = '<option value="">Všetky roky</option>';
        ...
        console.error('Chyba pri načítavaní rokov:', error);
    }
}
...
async function loadEntries(page = 1) {
    try {
        ...
        // Pre kategórie - ak je vybraná podkategória, použij ju, inak hlavnú
        ...
        // Povolenie/zakázanie mesiaca podľa roku
        ...
        console.error('Chyba pri načítavaní záznamov:', error);
        showAlert('Chyba pri načítavaní záznamov', 'danger');
    }
}
        ...
        entriesList.innerHTML = `
            <div class="card">
                <div class="card-body text-center py-5">
                    ...
                    <h5 class="text-muted">Žiadne záznamy</h5>
                    <p class="text-muted">Skús zmeniť filtre alebo pridať nový záznam</p>
                </div>
            </div>
        `;
        return;
        ...
        <div class="card entry-card fade-in">
                    ...
                    <div>
                        ...
                        ${entry.category ? `
                            <div class="entry-category">
                                ${entry.category.icon || '📝'} ${entry.category.display_name || entry.category.name}
                            </div>
                        ` : ''}
                    </div>
                </div>
            ...
            <div class="entry-content">
                ...
                ${entry.attachments && entry.attachments.length > 0 ? `
                    <div class="entry-attachments mt-2">
                        <strong><i class="fas fa-paperclip"></i> Prílohy:</strong>
                        <div class="mt-1">
                            ${entry.attachments.map(att => `
                                ...
                                ${att.original_filename.toLowerCase().endsWith('.pdf') ? `
                                  <a href="/viewer/pdf/${att.id}" target="_blank" class="btn btn-sm btn-outline-primary me-1 mb-1">
                                    <i class="fas fa-eye"></i> Zobraziť
                                  </a>
                                ` : ''}
                                <button type="button" class="btn btn-sm btn-outline-success me-1 mb-1" onclick="openInReader(${att.id})">
                                  <i class="fas fa-external-link-alt"></i> Otvoriť v čítačke
                                </button>
                            `).join('')}
                        </div>
                    </div>
            ...
            <div class="entry-actions">
                <button class="btn btn-warning btn-sm" onclick="editEntry(${entry.id})">
                    <i class="fas fa-edit"></i> Editovať
                </button>
...
// Zobrazenie formulára pre pridanie záznamu
function showAddEntryForm() {
    try {
        editingEntryId = null;
        document.getElementById('entryModalTitle').textContent = 'Pridať záznam';
        document.getElementById('entryForm').reset();
        setCurrentDateTime();
        
        // Resetovať kategórie
        document.getElementById('entryMainCategory').value = '';
        document.getElementById('entrySubcategory').innerHTML = '<option value="">Vyber podkategóriu</option>';
        ...
        console.error('Chyba pri otváraní formulára:', error);
        showAlert('Chyba pri otváraní formulára: ' + error.message, 'danger');
    }
}
...
async function editEntry(entryId) {
    try {
        ...
        if (data.entry) {
            ...
            document.getElementById('entryModalTitle').textContent = 'Editovať záznam';
            ...
            // Nastavenie kategórie - musíme zistiť či je to hlavná alebo podkategória
            if (entry.category_id) {
                // Načítaj informácie o kategórii
            ...
            // Vyčistiť file input pri editácii (nechceme staré súbory)
            document.getElementById('entryAttachments').value = '';
        ...
        console.error('Chyba pri načítavaní záznamu:', error);
        showAlert('Chyba pri načítavaní záznamu', 'danger');
    }
}
...
async function saveEntry() {
    try {
        ...
        // Použij podkategóriu ak je vybraná, inak hlavnú kategóriu
        ...
        if (!formData.title || !formData.content || !formData.category_id) {
            showAlert('Vyplň všetky povinné polia', 'warning');
            return;
        }
        ...
        if (response.ok) {
            ...
            // Nahrať prílohy ak sú vybrané - ČAKAJ NA DOKONČENIE!
            ...
            if (fileInput.files.length > 0) {
                ...
                if (uploadResult.successCount > 0) {
                    successMessage += ` (${uploadResult.successCount} príloha/príloh nahrané)`;
                }
                if (uploadResult.failCount > 0) {
                    showAlert(`⚠️ ${uploadResult.failCount} príloha/príloh sa nepodarilo nahrať`, 'warning');
                }
            }
            ...
            // Zatvorenie modalu a reset formulára
            const modalElement = document.getElementById('entryModal');
        ...
        showAlert('Chyba pri ukladaní záznamu', 'danger');
    }
}

// Nahrať prílohy k záznamu
async function uploadAttachments(entryId, files) {
    ...
    for (let file of files) {
        try {
            ...
            console.log(`Nahrávam súbor: ${file.name}`);
            ...
            console.log(`Odpoveď servera pre ${file.name}: ${response.status}`);
            
            if (response.ok) {
                successCount++;
                console.log(`✓ Súbor ${file.name} bol úspešne nahraný`);
            } else {
                failCount++;
                const error = await response.json();
                console.error(`✗ Chyba pri nahrávaní ${file.name}: ${error.error}`);
            }
        } catch (error) {
            failCount++;
            console.error(`✗ Chyba pri nahrávaní ${file.name}:`, error);
        }
    }
    
    console.log(`Nahrávanie dokončené: ${successCount} úspešne, ${failCount} zlyhal`);
...
async function deleteEntry(entryId) {
    ...
    try {
        ...
        console.error('Chyba pri mazaní záznamu:', error);
        showAlert('Chyba pri mazaní záznamu', 'danger');
    }
}
...
async function showStats() {
    try {
        ...
        showAlert('Chyba pri načítavaní štatistík', 'danger');
    }
}
...
function displayStats(stats, year) {
    ...
    statsContent.innerHTML = `
        <div class="row">
            <div class="col-md-6">
                <div class="stats-card">
                    ...
                    <span class="stats-label">Celkom záznamov</span>
                </div>
            </div>
            <div class="col-md-6">
                <div class="stats-card">
                    <span class="stats-number">${stats.year_entries}</span>
                    <span class="stats-label">Záznamy${yearText}</span>
                </div>
            </div>
        </div>
        ...
        ${stats.categories.length > 0 ? `
            <div class="mt-4">
                <h6>Záznamy podľa kategórií${yearText}</h6>
        ...
        ${stats.months.length > 0 ? `
            <div class="mt-4">
                <h6>Záznamy podľa mesiacov${yearText}</h6>
                <div class="row">
...
async function saveManageCategory() {
    try {
        const formData = {
            ...
            icon: document.getElementById('manageCategoryIcon').value || '📝',
        ...
        if (response.ok) {
            ...
            resetManageForm();
            loadManageCategories();
            loadCategories(); // Obnoviť hlavný zoznam kategórií
        } else {
            showAlert(result.error || 'Chyba pri ukladaní', 'danger');
        }
    } catch (error) {
...
function editManageCategory(categoryId) {
    ...
    if (category) {
        ...
        document.getElementById('categoryFormTitle').textContent = 'Editovať kategóriu';
    }
}
...
function resetManageForm() {
    ...
    document.getElementById('manageCategoryIcon').value = '📝';
    document.getElementById('manageCategoryColor').value = '#4CAF50';
    document.getElementById('manageCancelEdit').style.display = 'none';
    document.getElementById('categoryFormTitle').textContent = 'Pridať kategóriu';
}
...
// Funkcia pre stiahnutie prílohy
function downloadAttachment(attachmentId, filename) {
    // Pre PDF súbory otvoriť v novom okne, pre ostatné stiahnuť
    if (filename.toLowerCase().endsWith('.pdf')) {
        ...
        alert("Súbor bol stiahnutý. Nájdete ho v priečinku Stiahnuté (Downloads) vo vašom počítači.");
    }
}

function openInReader(attachmentId) {
    fetch(`/api/attachments/${attachmentId}/open`, {method: 'POST'})
        .then(resp => resp.json())
        .then(data => {
            if (data.status === 'ok') {
                let msg = `Súbor sa pokúša otvoriť v aplikácii: ${data.launcher || 'neznáme'}`;
                if (data.file_path) msg += `\nCesta: ${data.file_path}`;
                console.log('openInReader response:', data);
                // Nebudeme zobrazovať alert, len konzolu
            } else {
                alert('Chyba: ' + (data.error || 'Neznáma chyba'));
            }
        })
        .catch(err => {
            alert('Chyba pri spúšťaní lokálnej čítačky: ' + err.message);
        });
}

function openFolder(attachmentId) {
    fetch(`/api/attachments/${attachmentId}/open_folder`, {method: 'POST'})
        .then(resp => resp.json())
        .then(data => {
            if (data.status === 'ok') {
                alert('Priečinok s prílohami sa otvorí vo vašom správcovi súborov.');
            } else {
                alert('Chyba: ' + (data.error || 'Neznáma chyba'));
            }
        })
        .catch(err => {
            alert('Chyba pri otváraní priečinka: ' + err.message);
        });
}
